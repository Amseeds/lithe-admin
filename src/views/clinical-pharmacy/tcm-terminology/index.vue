...
import { getList, type TcmStandard, type StandardQueryParams } from '@/api/TCM/standard.ts'
...
// 查询参数
const [queryParams, , resetQueryParams] = useResettableReactive<StandardQueryParams>({
  keyword: '',
  categoryId: null,
  pageNum: 1,
  pageSize: 10,
...
// 数据列表
const dataList = ref<TcmStandard[]>([])
...
// 表格列
const columns = [
  { title: '标题', key: 'originalName' },
  {
    title: '操作',
    key: 'actions',
    width: 200,
    render: (row: TcmStandard) => (
      <NSpace>
        <NButton
          text
          type='primary'
...
// 下载
const handleDownload = (row: TcmStandard) => {
  console.log('下载资料:', row)
  download({
    url: `/api/standards/${row.id}/download`,
  })
}
        ...
        >
          ...
          <NFormItem label="标准分类">
            <NSelect
              ...
              :options="[
                { label: '中医病症诊疗疗效标准', value: '1' },
                { label: '中医基础理论术语', value: '2' },
                { label: '中医临床诊疗术语(疾病)', value: '3' },
                { label: '中医临床诊疗术语(证候)', value: '4' },
                { label: '中医临床诊疗术语(治法)', value: '5' },
              ]"
              style="width: 200px"
              clearable
